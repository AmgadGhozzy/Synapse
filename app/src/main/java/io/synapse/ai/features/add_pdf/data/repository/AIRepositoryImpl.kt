package io.synapse.ai.features.add_pdf.data.repository
...
import io.synapse.ai.features.add_pdf.data.remote.GeneratePackRequest
import io.synapse.ai.features.add_pdf.data.sse.SseEventParser
import io.synapse.ai.features.add_pdf.domain.model.GenerationConfig
import io.synapse.ai.features.add_pdf.domain.model.GenerationError
    ...
    override fun generatePackStream(
        ...
        try {
            val request = GeneratePackRequest(
                ...
                pageCount     = generationConfig.pageCount,
            )
    ...
    private fun mapHttpError(status: Int, body: String): GenerationError {
        ...
        return when (status) {
            400  -> when (dto.code) {
                ...
                "PDF_TOO_MANY_PAGES"               -> GenerationError.FileTooLarge(dto.error)
            ...
            429  -> {
                when (dto.code) {
                    "QUOTA_MONTHLY_LIMIT" -> GenerationError.MonthlyLimitExceeded(dto.error)
                    else -> GenerationError.DailyLimitExceeded(dto.error)
                }
            }
