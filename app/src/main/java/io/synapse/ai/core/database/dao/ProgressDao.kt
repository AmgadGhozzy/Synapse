...
import io.synapse.ai.core.database.entity.PackReviewedDay
import io.synapse.ai.core.database.entity.QuestionProgressEntity
...
interface ProgressDao {
    ...
    @Query(
        """
        SELECT DISTINCT (qp.lastReviewed / 86400000) FROM question_progress qp
        INNER JOIN questions q ON q.id = qp.questionId
        WHERE q.packId = :packId AND qp.lastReviewed IS NOT NULL
        """
    )
    ...
    @Query(
        """
        SELECT DISTINCT q.packId, (qp.lastReviewed / 86400000) AS reviewedEpochDay
        FROM question_progress qp
        INNER JOIN questions q ON q.id = qp.questionId
        WHERE qp.lastReviewed IS NOT NULL
        """
    )
    suspend fun getAllReviewedEpochDays(): List<PackReviewedDay>
    ...
    /**
     * Count of cards due for review on or before [endOfToday].
     * Excludes soft-deleted questions. Cards with no progress entry are NOT
     * counted here — they are handled separately by [QuestionDao.getDueQuestionsJoined].
     * This is intentionally scoped to existing SRS-tracked cards only.
     */
    @Query(
        """
        SELECT COUNT(*) FROM question_progress qp
        INNER JOIN questions q ON q.id = qp.questionId
        WHERE qp.nextReview <= :endOfToday
          AND q.isDeleted = 0
        """
    )
    suspend fun getDueCount(endOfToday: Long): Int

    @Query("UPDATE question_progress SET isDirty = 0 WHERE questionId IN (:ids)")
    suspend fun clearDirtyFlags(ids: List<Long>)

    @Query("SELECT * FROM question_progress WHERE isDirty = 1")
    suspend fun getDirtyProgress(): List<QuestionProgressEntity>
}
