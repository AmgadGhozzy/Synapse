...
import io.synapse.ai.core.database.entity.SessionQuestionCrossRef
import io.synapse.ai.core.database.entity.SessionWithQuestions
import io.synapse.ai.core.database.entity.StudySessionEntity
import io.synapse.ai.domains.study.model.DayActivityModel
...
interface SessionDao {
    ...
    @Query("SELECT * FROM study_sessions WHERE remoteId = :remoteId LIMIT 1")
    suspend fun getSessionByRemoteId(remoteId: String): StudySessionEntity?
    ...
    @Query("UPDATE study_sessions SET isDirty = 0 WHERE id IN (:ids)")
    suspend fun clearDirtyFlags(ids: List<Long>)

    @Query("SELECT * FROM study_sessions WHERE isDirty = 1")
    suspend fun getDirtySessions(): List<StudySessionEntity>
}
